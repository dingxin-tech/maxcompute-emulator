...
docker build --platform linux/amd64 --target prebuilt \
  -t maxcompute-emulator:2.0.0-ck.1 .
mvn -B -f tests/java/pom.xml test
docker save maxcompute-emulator:2.0.0-ck.1 | gzip > maxcompute-emulator-2.0.0-ck.1-linux-amd64.tar.gz
shasum -a 256 maxcompute-emulator-2.0.0-ck.1-linux-amd64.tar.gz
```
